...
const handle_update_Close = () => {set_update_Show(false);
    setForm({
        id:0,
        name: "",
        last_name:"",
...
const handle_terminate_Close = () => {set_terminate_Show(false);
    setForm({
        id:0,
        name: "",
        last_name:"",
...
const handle_activate_Close = () => {set_activate_Show(false);
    setForm({
        id:0,
        name: "",
        last_name:"",
...
const handle_delete_Close = () => {set_delete_Show(false);
    setForm({
        id:0,
        name: "",
        last_name:"",
...
useEffect(() => {
   ...
   // By Role
       fetch(`${URL}/api/auth/user/role?id=${formValue.user_role_id}`,requestOptions)
                        ...
                        <tbody>{
                            ActiveUsersData.map((user, Index)=>{
                                return<tr key={Index}>
                                    ...
                                    <td>  <Form.Select size="sm"  value={user.is_admin} disabled>
                ...
                <Nav  className="justify-content-start">
                 ...
                 <Form.Group as={Col} className="mb-3" >
                                    <div className="form-group dropdown">
                                        <select className="form-control" name="user_role_id" id="user_role_id" onChange={handleChange} value={formValue.user_role_id} required>
 ...
 <Modal show={show_update_user} onHide={handle_update_Close}>
                    ...
                    <Modal.Body>
                        <Form onSubmit={handleUpdateUser}>
                                ...
                                <InputGroup className="mb-3">
                                  <InputGroup.Text  className="col-4" id="name">Last Name :</InputGroup.Text>
                                  <FormControl
                                    type="text"
                                    aria-label="name"
                                    aria-describedby="name"
                                ...
                                <Form.Group className="mb-3" >
                                    ...
                                    <div className="form-group dropdown">
                                        <select className="form-control" name="is_admin" id="is_admin" onChange={handleChange} value={formValue.is_admin} required>
                                ...
                                <Form.Group className="mb-3" >
                                    ...
                                    <div className="form-group dropdown">
                                        <select className="form-control" name="team_id" id="team_id" onChange={handleChange} value={formValue.team_id} required>
            ...
            {/* Users Termination */}
                    <Modal  show={show_terminate_user}  onHide={handle_terminate_Close} backdrop="static"  keyboard={false}  >
                            <Modal.Header closeButton>
                                <Modal.Title>Termination Confirmation </Modal.Title>
                            </Modal.Header>
                            <Modal.Body>
                            please note your are about to terminate  <b>{formValue.name}</b>'s acccount from the system.
                            this account can no longer access the services of the system.
                            </Modal.Body>
                            <Modal.Footer>
                                <Button size='sm' variant="secondary" onClick={handle_terminate_Close}>
                                No
                                </Button>{ ' '}{' '}
                                <Button size='sm' variant="danger" onClick={handleUserTerminateSubmit}>Yes</Button>
                            </Modal.Footer>
                            </Modal>
            {/* Users Activation */}
                    <Modal  show={show_activate_user}  onHide={handle_activate_Close} backdrop="static"  keyboard={false}  >
                        <Modal.Header closeButton>
                            <Modal.Title>Remove User Role </Modal.Title>
                        </Modal.Header>
                        <Modal.Body>
                        please note your are about activate <b>{formValue.name}</b>'s from the system.
                        this account will gain access to services.
                        </Modal.Body>
                        <Modal.Footer>
                            <Button size='sm' variant="secondary" onClick={handle_activate_Close}>
                            No
                            </Button>{ ' '}{' '}
                            <Button size='sm' variant="success" onClick={handleUserActivateSubmit}>Yes</Button>
                        </Modal.Footer>
                </Modal>

                           {/* Users Account Removal */}
          <Modal  show={show_delete_user}  onHide={handle_delete_Close} backdrop="static"  keyboard={false}  >
                  <Modal.Header closeButton>
                    <Modal.Title>Remove User Role </Modal.Title>
                  </Modal.Header>
                  <Modal.Body>
                  please note your are about remove <b>{formValue.name}</b>'s account from the system.
              ...
              {/* Successfully Active */}
              <Toast onClose={handleCloseSuccessActive} show={success_active} bg={'success'} delay={5000} autohide>
                        <Toast.Header>
                          <img
                            src="holder.js/20x20?text=%20"
                            className="rounded me-2"
                        ...
                        <Toast.Body className='text-white'>  Successfully  Activated </Toast.Body>
                      </Toast>
              {/* Successfully Deactived */}
              <Toast onClose={handleCloseSuccessDeActive} show={success_de_activated} bg={'warning'} delay={5000} autohide>
                        <Toast.Header>
                          <img
                            src="holder.js/20x20?text=%20"
                            className="rounded me-2"
                        ...
                        <Toast.Body className='text-white'>Terminated Successfully</Toast.Body>
                      </Toast>

                {/* Error   */}
                <Toast onClose={handleCloseError} show={error_occured} bg={'warning'} delay={5000} autohide>
                        <Toast.Header>
                          <img
                            src="holder.js/20x20?text=%20"
                            className="rounded me-2"
