...
function ProjectManager() {
  ...
  const [
    over_due_assign_all_projects,
    set_over_due_assign_all_projects_ProjectData,
  ] = useState([]);
  ...
  const [old_project_assignment_data, set_old_project_assignment_data] =
    useState([]);
  const [
    old_over_due_project_assignment_data,
    set_old_over_due_project_assignment_data,
  ] = useState([]);
  ...
  const latest__project_assignment = useMemo(
    () => old_project_assignment_data,
    [old_project_assignment_data]
  );
  const latest_over_due_project_assignment = useMemo(
    () => old_over_due_project_assignment_data,
    [old_over_due_project_assignment_data]
  );
  ...
  useEffect(() => {
    ...
    const activeoverview = setInterval(() => {
      fetch(
        `${URL}/api/auth/project_assignment/over_due/dev/all?team_id=${localStorage.getItem(
          "team"
        )}&id=${localStorage.getItem("SUID")}`,
        requestOptions
      )
            ...
            <Card.Body className="dev-project-card ">
              <Tabs defaultActiveKey="open" className="mb-3">
                ...
                <Tab eventKey="over due" title="Over Due">
                  <Tabs
                    defaultActiveKey="over due bet_projects"
                    className="mb-3"
                  >
                    ...
                    <Tab eventKey="over due all" title="All">
                      <Table size="sm" striped bordered hover>
                        ...
                        <tbody>
                          {over_due_assign_all_projects.map(
                            (project, Index) => {
                              return (
                                <tr key={Index}>
                                  <td>{project.project.name}</td>
          ...
          <Form onSubmit={handleSubmitUpdateProject}>
            <InputGroup className="mb-3">
              ...
              <Form.Select
                ...
                value={AssignProjectFormValue.user_status_id}
                required
              >
