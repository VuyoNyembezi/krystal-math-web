...
const handleClose_update_Project = () => {setShow_Update_Project(false);
  setProjectFormValue({
    name:"",
    team_id:0,
    user_id:0,
 ...
 function selectProject(project){
  setProjectFormValue({
   ...
   function handle_Search_Project_Submit(event){
    ...
    fetch(`${URL}/api/auth/projects/strategic/search?search=${search_key.project_search}`,requestOptions)
    .then(response => response.json())
    .then(Result => {
...
.then((response) => {
  ...
  if(response.status === 200){
    ...
    handleClose_update_Project();
     ...
     return(
         <>
          <Tab.Container id="left-tabs-example" defaultActiveKey="strategic_bet_project" >
         <Tab.Content>
         <CardGroup>
           <Card>
             <Card.Header>
                 <h6>Strategic Projects</h6>
                 <Button variant="outline-warning" size='sm'  onClick={handleShowAddProject}>Add Project</Button>
                 <br/>
                                ...
                                <tbody>
                                  ...
                                  BetSoftwareProjectData.map((project, key)=>{
                                    return <tr key={key}>
                                      ...
                                      <td className='text-center'><button className="btn" size='sm' onClick={() => selectProject(project)}><Button variant="outline-success" size='sm' onClick={handleShow_update_Project}>Update</Button></button></td>
                                    </tr>
                                  })
                                  } 
 ...
 {/* Projects */}
 <Modal show={showAddProject} onHide={handleCloseAddProject} animation={false}>
 ...
 {/* Update modal for Integrations */}
 <Modal show={show_update_Project} onHide={handleClose_update_Project} animation={false}>
         <Modal.Header closeButton>
       ...
       <FormGroup>
         ...
         <Modal.Footer>
           <Button variant="secondary" size='sm' onClick={handleClose_update_Project}>
             Close
           </Button> 
