...
function OperationalProjects() {
  ...
  const handleClose_update_Uproject = () => {
    setShow_Update_Project(false);
    setProjectFormValue({
      id: 0,
      name: "",
      team_id: 0,
  ...
  useEffect(() => {
    ...
    if (
      ...
      const activeoverview = setInterval(() => {
        fetch(`${URL}/api/auth/projects/operational`, requestOptions)
          .then((response) => response.json())
          .then((results) => {
  ...
  function handle_Search_Project_Submit(event) {
    ...
    const activeoverview = setInterval(() => {
      fetch(
        `${URL}/api/auth/projects/operational/search?search=${search_key.project_search}`,
        requestOptions
      )
        .then((response) => response.json())
        .then((Result) => {
  ...
  return (
    <>
                            ...
                            <td className="text-center">
                              ...
                              >
                                <Button
                                  ...
                                  size="sm"
                                  onClick={handleShow_update_Project}
                                >
                                  Update
                                </Button>
                              </button>
      ...
      <Modal
        show={show_update_Project}
        onHide={handleClose_update_Uproject}
        animation={false}
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Update <b>{projectFormValue.name}</b>
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
        ...
        <Modal.Footer>
          <Button
            variant="secondary"
            size="sm"
            onClick={handleClose_update_Uproject}
          >
            Close
          </Button>
        </Modal.Footer>
